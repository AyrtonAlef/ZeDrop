function EvalContIncDrop()
%EVALCONTINCDROP Evaluation of of continuous inclined drop quasi-static 
% experiment.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Code used for the analysis of continuous inclined drop quasi-static 
% experiment. Based on Example1.m and Example2.m codes from Andersen and 
% Taboryski 2017.
...
%                         0. Reset settings
%---------------------------------------------------------------------------
if ~isfile('VarContIncDrop.mat') %Initialize default settings
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    save('VarContIncDrop.mat',routVarNames{:});
end
%}
%%
routVarNames = {'EnhanceContrast','low_in','high_in','low_out','high_out','SmoothingFilter','sigmaSmoothFilter','sizeSmoothFilter','EdgeDetMethodOpt','edgeDetMetName','edgeDetDirection','edgeDetSigma','edgeDetThreshold','edgeDetFilter','edgeDetFilterTypeOpt','edgeDetFilterTypeName','edgeDetFilterSize','edgeDetFilterSigma','edgeDetFilterShape','edgeDetDifThreshold','edgeDetOrder','edgeDetSmoothingIter','margin','jump_dist','filterMode','MorphOpen','openP','defaultopenP','openConn','MorphBridge','bridgeN','MorphClose','closeN','MorphThin','thinN','BaselineDetOpt','yDistWeight','marginBaselineDet','yAverageLR','nBaselineDet','cutBaselineDet','percentBaselineDet','fillBlankHorLines','marginMaskBaselineDet','maskSize','limBaselineTilt','plotIntEdgeDetImage','UseAverageBaseVec','UseCircFit','n_circfit','PlotCircleImages','UsePolyFit','n_polyfit','poly_ord','PlotPolynomialImages','UsePolyFitDropen','n_polyfitDropen','poly_ordDropen','PlotPolynomialDropenImages','UseEllipseFit','PlotEllipticImages','UseMaskMethod','PlotMaskImages','plotIntFittingImages'};
fprintf('------------------- EVALUATION OF CONTINUOUS INCLINED DROP EXPERIMENT ------------------ \n');
%---------------------------------------------------------------------------
%                1.       Loading of data
%---------------------------------------------------------------------------
%- Load variables
load VarContIncDrop.mat
%- Read files
...
tiltExp = zeros(nImages,1); %Tilt angle in °
timeExp = zeros(nImages,1); %Time in s

%- Loading images, tilt angle and time informations
%-- Load tilt angle [°] information
fprintf('Tilting angle information... \n');
ansTiltDirection = input('Experiment tilting direction (clockwise[CW] or counterclockwise[CCW]) [CW]: ','s');
if isempty(ansTiltDirection) || (~strcmp(ansTiltDirection,"CW") && ~strcmp(ansTiltDirection,"CCW"))
    ansTiltDirection = "CW";
end
%--- Determination of signs for triple line velocity calculation
if strcmp(ansTiltDirection,"CCW") %Counterclockwise rotation
    sign = -1;
    nameL = 'downhill';
    nameR = 'uphill';
...
ansTilt = input('Load tilting angle information from the image name (I), from a file (F) or enter it manually (M) [I]? ','s');
if isempty(ansTilt) || (ansTilt ~= 'I' && ansTilt ~= 'F' && ansTilt ~= 'M')
    ansTilt = 'I';
end
if ansTilt == 'F'
    fprintf('Select tilting angle information file... \n')
    [fileNameTilt, inputPathTilt] = uigetfile({'*.dat;*.csv;*.txt;*.LOG','Data File(*.dat,*.csv,*.txt,*.LOG)';'*.*','All Files (*.*)'},'Select a file',inputPath);
    fullfileNameTilt = fullfile(inputPathTilt,fileNameTilt);
    fprintf('Reading tilting angle information file... \n');
    DataTilt = readtable(fullfileNameTilt,'VariableNamingRule','preserve'); %Import data from file
    col = input('Enter the column which contain the tilting data [3]: ');
    if isempty(col)
        col = 3;
    end
    tiltExp = abs(DataTilt{:,col}); %Extract tilting angle information
elseif ansTilt == 'M'
    tiltIni = input('Initial tilt angle (°) [0]: ');
    if isempty(tiltIni)
        tiltIni = 0;
    end
    tiltStep = input('Tilt interval between images (°) [0.2]: ');
    if isempty(tiltStep)
        tiltStep = 0.2;
    end
    fprintf('Creating tilt angle information ... \n');
    tiltExp = transpose(linspace(tiltIni,tiltIni+(tiltStep*nImages),nImages));    
end
%-- Load time [s] information
fprintf('Time information... \n');
ansTime = input('Load time information from the image name (I), from a file (F) or enter it manually (M) [I]? ','s');
if isempty(ansTime) || (ansTime ~= 'I' && ansTime ~= 'F' && ansTime ~= 'M')
    ansTime = 'I';
end
if ansTime == 'F'
    fprintf('Select time information file... \n')
    [fileNameTime, inputPathTime] = uigetfile({'*.dat;*.csv;*.txt;*.LOG','Data File(*.dat,*.csv,*.txt,*.LOG)';'*.*','All Files (*.*)'},'Select a file',inputPath);
    fullfileNameTime = fullfile(inputPathTime,fileNameTime);
    fprintf('Reading time information file... \n');
    DataTime = readtable(fullfileNameTime,'VariableNamingRule','preserve'); %Import data from file
    col = input('Enter the column which contain the time data [2]: ');
    if isempty(col)
        col = 2;
    end
    timeExp = DataTime{:,col}; %Extract time information
elseif ansTime == 'M'
    timeStep = input('Time interval between images (s) [1]: ');
    if isempty(timeStep)
        timeStep = 1;
    end
    fprintf('Creating time information ... \n');
    timeExp = transpose(linspace(0,timeStep*nImages,nImages));
end
%-- Load images information
fprintf('Reading images... \n');
for i = 1:nImages
    im{i} = imread([inputPath,char(fileNames(i))]); %Reading images
    if ansTilt == 'I' %Extract tilt information from image name
        extractTilt = extractBetween(fileNames{i},"alpha(°)_","_"); %Extract tilt angle information from file name
        tiltExp(i) = str2num(extractTilt{1}); %Convert extracted string to tilt angle value in °
    end
    if ansTime == 'I' %Extract time information from image name
        extractTime = extractBetween(fileNames{i},"time(s)_","_"); %Extract time information from file name
        timeExp(i) = str2num(extractTime{1}); %Convert extracted string to time value in s
    end
end
...
while 1
    ...
    for i = 1:nImages
        ...
        if mod(i,plotIntEdgeDetImages) == 0 || i == 1
            ...
            plot(edgeL.x,edgeL.y,':r','LineWidth',2,'DisplayName',strcat(nameL," edge"))
            plot(edgeR.x,edgeR.y,':b','LineWidth',2,'DisplayName',strcat(nameR," edge"))
            %-- Display contact points
            plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
            plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
            %-- Display legend
...
while 1
    ...
    tiltVel = NaN(nImages,1); %Tilting angle for the calculation of triple line velocity
    ...
    for i = 1:nImages
        ...
        if y0L ~= y0R
            ...
            tiltVel(i,1) = tiltExp(i); %Tilting angle for the calculation of triple line velocity
            ...
            fprintf('%s x displacement = %.2f mm \n',nameL,dispCPmm(i,1));
            fprintf('%s x displacement = %.2f mm \n',nameR,dispCPmm(i,2));
            ...
            if UsePolyFitDropen
                ...
                fprintf('Polynomial fitting (Dropen) %s = %.2f° \n',nameL,PolyDropenCAS(i,1));
                fprintf('Polynomial fitting (Dropen) %s = %.2f° \n',nameR,PolyDropenCAS(i,2));
            end
            %- Polynomial fitting
            if UsePolyFit
                ...
                fprintf('Polynomial fitting %s = %.2f° \n',nameL,PolyCAS(i,1));
                fprintf('Polynomial fitting %s = %.2f° \n',nameR,PolyCAS(i,2));
            end
            ...
            if UseEllipseFit
                ...
                fprintf('Ellipse fitting %s = %.2f° \n',nameL,EllipseCAS(i,1));
                fprintf('Ellipse fitting %s = %.2f° \n',nameR,EllipseCAS(i,2));
            end
            %- Mask method
            if UseMaskMethod
        ...
        fprintf('Estimated volume (spherical assumption) = %.2f uL \n', volEstSph(i,1));
            ...
            if mod(i,plotIntFittingImages) == 0 || i == 1
                ...
                if PlotCircleImages
                    ...
                    plot((x0R-x0L)*t+x0L,(y0R-y0L)*t+y0L,'r--','LineWidth',2,'DisplayName','Baseline')
                    %-- Plot fitted edge
                    plot(CircleData.EvalCircL(:,1),CircleData.EvalCircL(:,2),':r','LineWidth',2,'DisplayName',strcat(nameL," circle fitted edge"))
                    plot(CircleData.EvalCircR(:,1),CircleData.EvalCircR(:,2),':b','LineWidth',2,'DisplayName',strcat(nameR," circle fitted edge"))
                    %-- Plot fitted data closer to contact points
                    radius = 40;
                    tilt = atand((y0R-y0L)/(x0R-x0L));
                    plot([CircleData.TLL(2),CircleData.TLL(2)+radius*cosd(CircleData.CAL-tilt)],[CircleData.TLL(1),CircleData.TLL(1)-radius*sind(CircleData.CAL-tilt)],'LineStyle','--','LineWidth', 2,'color','g','DisplayName',strcat("Fitted data closer to ",nameL))
                    plot([CircleData.TLR(2),CircleData.TLR(2)-radius*cosd(CircleData.CAR+tilt)],[CircleData.TLR(1),CircleData.TLR(1)-radius*sind(CircleData.CAR+tilt)],'LineStyle','--','LineWidth', 2,'color','c','DisplayName',strcat("Fitted data closer to ",nameR))
                    %-- Display contact points
                    plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
                    plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
                    %-- Display legend
                ...
                if PlotPolynomialDropenImages
                    ...
                    plot((x0R-x0L)*t+x0L,(y0R-y0L)*t+y0L,'r--','LineWidth',2,'DisplayName','Baseline')
                    %-- Plot fitted edge
                    plot(PolyDataDropen.EvalPolyL(:,1),PolyDataDropen.EvalPolyL(:,2),':r','LineWidth',2,'DisplayName',strcat(nameL," poly (Dropen) fitted edge"))
                    plot(PolyDataDropen.EvalPolyR(:,1),PolyDataDropen.EvalPolyR(:,2),':b','LineWidth',2,'DisplayName',strcat(nameR," poly (Dropen) fitted edge"))
                    %-- Plot fitted data closer to contact points
                    radius = 40;
                    tilt = atand((y0R-y0L)/(x0R-x0L));
                    plot([PolyDataDropen.TLL(2),PolyDataDropen.TLL(2)+radius*cosd(PolyDataDropen.CAL-tilt)],[PolyDataDropen.TLL(1),PolyDataDropen.TLL(1)-radius*sind(PolyDataDropen.CAL-tilt)],'LineStyle','--','LineWidth', 2,'color','g','DisplayName',strcat("Fitted data closer to ",nameL))
                    plot([PolyDataDropen.TLR(2),PolyDataDropen.TLR(2)-radius*cosd(PolyDataDropen.CAR+tilt)],[PolyDataDropen.TLR(1),PolyDataDropen.TLR(1)-radius*sind(PolyDataDropen.CAR+tilt)],'LineStyle','--','LineWidth', 2,'color','c','DisplayName',strcat("Fitted data closer to ",nameR))
                    %-- Display contact points
                    plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
                    plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
                    %-- Display legend
                ...
                if PlotPolynomialImages
                    if ~isempty(PolyData.TLL) && ~isempty(PolyData.TLR) && ~isempty(PolyData.CAL) && ~isempty(PolyData.CAR) %Check that polynomial fitting converged to results
                        ...
                        plot((x0R-x0L)*t+x0L,(y0R-y0L)*t+y0L,'r--','LineWidth',2,'DisplayName','Baseline')
                        %-- Plot fitted edge
                        plot(PolyData.EvalPolyL(:,1),PolyData.EvalPolyL(:,2),':r','LineWidth',2,'DisplayName',strcat(nameL," polynomial fitted edge"))
                        plot(PolyData.EvalPolyR(:,1),PolyData.EvalPolyR(:,2),':b','LineWidth',2,'DisplayName',strcat(nameL," polynomial fitted edge"))
                        %-- Plot fitted data closer to contact points
                        radius = 40;
                        tilt = atand((y0R-y0L)/(x0R-x0L));
                        plot([PolyData.TLL(1),PolyData.TLL(1)+radius*cosd(PolyData.CAL-tilt)],[PolyData.TLL(2),PolyData.TLL(2)-radius*sind(PolyData.CAL-tilt)],'LineStyle','--','LineWidth', 2,'color','g','DisplayName',strcat("Fitted data closer to ",nameL))
                        plot([PolyData.TLR(1),PolyData.TLR(1)-radius*cosd(PolyData.CAR+tilt)],[PolyData.TLR(2),PolyData.TLR(2)-radius*sind(PolyData.CAR+tilt)],'LineStyle','--','LineWidth', 2,'color','c','DisplayName',strcat("Fitted data closer to ",nameR))
                        %-- Display contact points
                        plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
                        plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
                        %-- Display legend
                ...
                if PlotEllipticImages
                    if ~isnan(max(EllipseData.TLL)) && ~isnan(max(EllipseData.TLR)) && ~isnan(EllipseData.CAL) && ~isnan(EllipseData.CAR) %Check that locate an ellipse
                        ...
                        plot(EL(:,1),EL(:,2),':r','LineWidth',2,'DisplayName',strcat(nameL," ellipse fitted edge"));
                        plot(ER(:,1),ER(:,2),':b','LineWidth',2,'DisplayName',strcat(nameL," ellipse fitted edge"));
                        %-- Plot fitted data closer to contact points
                        radius = 40;
                        plot([EllipseData.TLL(1),EllipseData.TLL(1)+radius*cosd(EllipseData.CAL-tilt)],[EllipseData.TLL(2),EllipseData.TLL(2)-radius*sind(EllipseData.CAL-tilt)],'LineStyle','--','LineWidth', 2,'color','g','DisplayName',strcat("Fitted data closer to ",nameL))
                        plot([EllipseData.TLR(1),EllipseData.TLR(1)-radius*cosd(EllipseData.CAR+tilt)],[EllipseData.TLR(2),EllipseData.TLR(2)-radius*sind(EllipseData.CAR+tilt)],'LineStyle','--','LineWidth', 2,'color','c','DisplayName',strcat("Fitted data closer to ",nameR))
                        %-- Display contact points
                        plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
                        plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
                        %-- Display legend
                ...
                if PlotMaskImages
                    ...
                    plot(AXyL,AXxL,':r','LineWidth',2,'DisplayName',strcat(nameL," detected edge"))
                    plot(AXyR,AXxR,':b','LineWidth',2,'DisplayName',strcat(nameR," detected edge"))
                    %-- Plot fitted data closer to contact points
                    radius = 40;
                    tilt=atand((y0R-y0L)/(x0R-x0L));
                    plot([x0L,x0L+radius*cosd(maskCAL-tilt)],[y0L,y0L-radius*sind(maskCAL-tilt)],'LineStyle','--','LineWidth', 2,'color','g','DisplayName',strcat("Fitted data closer to ",nameL))
                    plot([x0R,x0R-radius*cosd(maskCAR+tilt)],[y0R,y0R-radius*sind(maskCAR+tilt)],'LineStyle','--','LineWidth', 2,'color','c','DisplayName',strcat("Fitted data closer to ",nameR))
                    %-- Display contact points
                    plot(x0L,y0L,'mx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameL," CP"))
                    plot(x0R,y0R,'yx','MarkerSize',10,'LineWidth',2,'DisplayName',strcat(nameR," CP"))
                    %-- Display legend
                    ...
                    figure('Name',figureName);
                    %- Plot s x alpha
                    plot(alphamL(3,:),alphamL(5,:),'r--*','DisplayName',strcat(nameL," edge"));
                    hold on
                    plot(alphamR(3,:),alphamR(5,:),'b--*','DisplayName',strcat(nameR," edge"));
                    plot(alphamL(3,indexCPL),alphamL(5,indexCPL),'o','MarkerEdgeColor',[0.6350 0.0780 0.1840],'MarkerSize',18,'DisplayName',strcat(nameL," CP")); %left contact point
                    plot(alphamR(3,indexCPR),alphamR(5,indexCPR),'o','MarkerEdgeColor',[0 0.4470 0.7410],'MarkerSize',18,'DisplayName',strcat(nameR," CP")); %right contact point
                    xlabel('s');
                    ylabel('\alpha [°]')
...
if nImages > 2
    ...
    for i = 2:nImages-1
        ...
        if (abs(xVel(i,1) - xVel(i-1,1)) == 0) || (abs(xVel(i,2) - xVel(i-1,2)) == 0)
            ...
            fL = (xVel(i,1) - xVel(i-1,1))/abs((xVel(i,1) - xVel(i-1,1))); %f for the left side(+1 for downhill movement and -1 for uphill movement)
            fR = (xVel(i,2) - xVel(i-1,2))/abs((xVel(i,2) - xVel(i-1,2))); %f for the right side (+1 for downhill movement and -1 for uphill movement)
        end
        %fL = (xVel(i,1) - xVel(i-1,1))/abs((xVel(i,1) - xVel(i-1,1))); %f for the left side(+1 for downhill movement and -1 for uphill movement)
        %dxdtiltL = ((BaseVec(i-1,1)*tiltExp(i-1) + BaseVec(i,1)*tiltExp(i) + BaseVec(i+1,1)*tiltExp(i+1))*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))*(BaseVec(i-1,1) + BaseVec(i,1) + BaseVec(i+1,1))) / ((tiltExp(i-1)^2 + tiltExp(i)^2 + tiltExp(i+1)^2)*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))^2);
        dxdtiltL = ((xVel(i-1,1)*tiltVel(i-1) + xVel(i,1)*tiltVel(i) + xVel(i+1,1)*tiltVel(i+1))*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))*(xVel(i-1,1) + xVel(i,1) + xVel(i+1,1))) / ((tiltVel(i-1)^2 + tiltVel(i)^2 + tiltVel(i+1)^2)*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))^2);
        %dydtiltL = ((BaseVec(i-1,2)*tiltExp(i-1) + BaseVec(i,2)*tiltExp(i) + BaseVec(i+1,2)*tiltExp(i+1))*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))*(BaseVec(i-1,2) + BaseVec(i,2) + BaseVec(i+1,2))) / ((tiltExp(i-1)^2 + tiltExp(i)^2 + tiltExp(i+1)^2)*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))^2);
        dydtiltL = ((yVel(i-1,1)*tiltVel(i-1) + yVel(i,1)*tiltVel(i) + yVel(i+1,1)*tiltVel(i+1))*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))*(yVel(i-1,1) + yVel(i,1) + yVel(i+1,1))) / ((tiltVel(i-1)^2 + tiltVel(i)^2 + tiltVel(i+1)^2)*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))^2);
        vel(i,1) = 1000 * (sign * fL * sqrt(dxdtiltL^2 + dydtiltL^2))/scale; %Triple line velocity for the left side in um/°
        
        %fR = (xVel(i,2) - xVel(i-1,2))/abs((xVel(i,2) - xVel(i-1,2))); %f for the right side (+1 for downhill movement and -1 for uphill movement)
        %dxdtiltR = ((BaseVec(i-1,3)*tiltExp(i-1) + BaseVec(i,3)*tiltExp(i) + BaseVec(i+1,3)*tiltExp(i+1))*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))*(BaseVec(i-1,3) + BaseVec(i,3) + BaseVec(i+1,3))) / ((tiltExp(i-1)^2 + tiltExp(i)^2 + tiltExp(i+1)^2)*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))^2);
        dxdtiltR = ((xVel(i-1,2)*tiltVel(i-1) + xVel(i,2)*tiltVel(i) + xVel(i+1,2)*tiltVel(i+1))*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))*(xVel(i-1,2) + xVel(i,2) + xVel(i+1,2))) / ((tiltVel(i-1)^2 + tiltVel(i)^2 + tiltVel(i+1)^2)*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))^2);      
        %dydtiltR = ((BaseVec(i-1,4)*tiltExp(i-1) + BaseVec(i,4)*tiltExp(i) + BaseVec(i+1,4)*tiltExp(i+1))*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))*(BaseVec(i-1,4) + BaseVec(i,4) + BaseVec(i+1,4))) / ((tiltExp(i-1)^2 + tiltExp(i)^2 + tiltExp(i+1)^2)*3 - (tiltExp(i-1) + tiltExp(i) + tiltExp(i+1))^2);
        dydtiltR = ((yVel(i-1,2)*tiltVel(i-1) + yVel(i,2)*tiltVel(i) + yVel(i+1,2)*tiltVel(i+1))*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))*(yVel(i-1,2) + yVel(i,2) + yVel(i+1,2))) / ((tiltVel(i-1)^2 + tiltVel(i)^2 + tiltVel(i+1)^2)*3 - (tiltVel(i-1) + tiltVel(i) + tiltVel(i+1))^2);
        vel(i,2) = 1000 * (sign * fR * sqrt(dxdtiltR^2 + dydtiltR^2))/scale; %Triple line velocity for the right side in um/°
    end
end
...
%- Ploting results for all data ((x Position [pixels] && x displacement [mm] && vel [um/°] && CA [°]) x Tilting angle [°])
plotName = strcat('Plot_xPosition&&dispCPmm&&CAxTiltAngle');
...
%-- Top plot (x Position [pixels] x tilting angle [°])
ax1 = nexttile;
plot(ax1,tiltExp,BaseVec(:,1)','--*','color','blue','DisplayName',nameL);
hold on
plot(ax1,tiltExp,BaseVec(:,3)','--x','color','red','DisplayName',nameR);
xlabel(ax1,'Tilting angle [°]')
...
%-- Medium plot ((x Displacement [mm] && velocity [um/°]) x tilting angle [°])
ax2 = nexttile;
plot(ax2,tiltExp,dispCPmm(:,1)','--*','color','blue','DisplayName',strcat("xDisp ",nameL));
hold on
plot(ax2,tiltExp,dispCPmm(:,2)','--x','color','red','DisplayName',strcat("xDisp ",nameR));
ylabel(ax2,'x Displacement [mm]')
xlabel(ax2,'Tilting angle [°]')
yyaxis right
plot(ax2,tiltExp,vel(:,1)',':*','color','cyan','DisplayName',strcat("vel ",nameL));
hold on
plot(ax2,tiltExp,vel(:,2)',':x','color','magenta','DisplayName',strcat("vel ",nameR));
ylabel(ax2,strcat('velocity [',char(181),'m/°]'),'Color','k')
ax2.YColor = 'k';
...
lgdTop.NumColumns = 1;
lgdTop.Location = 'northeastoutside';
%-- Bottom plot (CA x tilting angle)
ax3 = nexttile;
if UseCircFit
plot(ax3,tiltExp,CircleCAS(:,1)','--*','color','blue','DisplayName',strcat("CircleFit ",nameL));
hold on
plot(ax3,tiltExp,CircleCAS(:,2)','--*','color','red','DisplayName',strcat("CircleFit ",nameR));
end
if UsePolyFitDropen
plot(ax3,tiltExp,PolyDropenCAS(:,1)','--+','color','blue','DisplayName',strcat("PolyFitDropen ",nameL));
hold on
plot(ax3,tiltExp,PolyDropenCAS(:,2)','--+','color','red','DisplayName',strcat("PolyFitDropen ",nameR));
end
if UsePolyFit
plot(ax3,tiltExp,PolyCAS(:,1)','--square','color','blue','DisplayName',strcat("PolyFit ",nameL));
hold on
plot(ax3,tiltExp,PolyCAS(:,2)','--square','color','red','DisplayName',strcat("PolyFit ",nameR));
end
if UseEllipseFit
plot(ax3,tiltExp,EllipseCAS(:,1)','--o','color','blue','DisplayName',strcat("EllipseFit ",nameL));
hold on
plot(ax3,tiltExp,EllipseCAS(:,2)','--o','color','red','DisplayName',strcat("EllipseFit ",nameR));
end
if UseMaskMethod
plot(ax3,tiltExp,MaskCAS(:,1)','--^','color','blue','DisplayName',strcat("MaskMet ",nameL));
hold on
plot(ax3,tiltExp,MaskCAS(:,2)','--^','color','red','DisplayName',strcat("MaskMet ",nameR));
end
xlabel(ax3,'Tilting angle [°]')
...
varnames = {'Name','Time[s]','TiltingAngle[°]','BaselineTilt[°]',strcat('x0',nameL,'[pxs]'),strcat('x0',nameR,'[pxs]'),strcat('xDispCP',nameL,'[pxs]'),strcat('xDispCP',nameR,'[pxs]'),strcat('xDispCP',nameL,'[mm]'),strcat('xDispCP',nameR,'[mm]'),strcat('vel',nameL,'[um/°]'),strcat('vel',nameR,'[um/°]'),strcat('circleCA',nameL,'[°]'),strcat('circleCA',nameR,'[°]'),strcat('polyDropenCA',nameL,'[°]'),strcat('polyDropenCA',nameR,'[°]'),strcat('polyCA',nameL,'[°]'),strcat('polyCA',nameR,'[°]'),strcat('ellipseCA',nameL,'[°]'),strcat('ellipseCA',nameR,'[°]'),strcat('maskCA',nameL,'[°]'),strcat('maskCA',nameR,'[°]')}; %Nome das variáveis no arquivo txt
T = table(fileNames',round(timeExp(:,1),2),round(tiltExp(:,1),2),round(BaselineTilt(:,1),2),round(BaseVec(:,1),4),round(BaseVec(:,3),4),round(dispCP(:,1),4),round(dispCP(:,2),4),round(dispCPmm(:,1),4),round(dispCPmm(:,2),4),round(vel(:,1),4),round(vel(:,2),4),round(CircleCAS(:,1),4),round(CircleCAS(:,2),4),round(PolyDropenCAS(:,1),4),round(PolyDropenCAS(:,2),4),round(PolyCAS(:,1),4),round(PolyCAS(:,2),4),round(EllipseCAS(:,1),4),round(EllipseCAS(:,2),4),round(MaskCAS(:,1),4),round(MaskCAS(:,2),4),'VariableNames',varnames);
